#!/bin/bash
# Docker Buildx Build Script for recommender
...
Build Docker image for $recommender using buildx.
...
echo -e "${BLUE}📦 Building $recommender...${NC}"
...
BUILD_CMD="$BUILD_CMD --tag=$REGISTRY/$recommender:latest"
BUILD_CMD="$BUILD_CMD --tag=$REGISTRY/$recommender:$TIMESTAMP"
...
if eval "$BUILD_CMD"; then
    echo ""
    echo -e "${GREEN}✅ Successfully built $recommender${NC}"
    echo -e "  Image: $REGISTRY/$recommender:latest"
    echo -e "  Image: $REGISTRY/$recommender:$TIMESTAMP"
else
    echo ""
    echo -e "${RED}❌ Failed to build $recommender${NC}"
    exit 1
fi
